...
void OLED_ShowFloat(uint8_t Line, uint8_t Column, float Number, uint8_t IntLength, uint8_t FraLength)
{
    ...
    float temp;
    uint8_t sign_pos = Column;  // 记录符号位置
    
    // 先清除整个显示区域（符号位+整数+小数点+小数）
    for(uint8_t i = 0; i < IntLength + FraLength + 2; i++) {
    ...
    // 提取小数部分
    temp = Number - (float)intPart;
    for (uint8_t i = 0; i < FraLength; i++)
    {
        temp *= 10.0f;
    }
    fraPart = (uint32_t)(temp + 0.5f); // 四舍五入
    
    // 显示小数部分
    OLED_ShowNum(Line, Column + IntLength + 1, fraPart, FraLength);
}
